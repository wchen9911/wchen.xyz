  ...
  return (
    <main className="min-h-screen bg-[#fdfdfc] dark:bg-[#121212] text-[#2d2d2d] dark:text-[#e4e4e7] font-sans selection:bg-[#a82a2a] selection:text-white transition-colors duration-500 overflow-x-hidden">
            ...
            <div className="relative group">
              <div className="relative w-64 h-[20rem] rounded-[2.5rem] bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800 p-8 flex flex-col justify-between overflow-hidden shadow-sm">
                 ...
                 <div className="space-y-1">
                    ...
                    <p className="text-base font-bold text-zinc-700 dark:text-zinc-300">Engineering @ YouTube</p>
                 </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-12 px-8">
        <div className="max-w-6xl mx-auto bg-[#f7f7f5] dark:bg-[#1c1c1c] rounded-[3rem] p-10 space-y-10 border border-zinc-100 dark:border-zinc-900">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2">
              <p className="text-[#a82a2a] font-bold text-xs uppercase tracking-widest">Principles</p>
              <h2 className="text-3xl font-extrabold tracking-tight">Management Philosophy</h2>
            </div>
            <p className="max-w-xs text-zinc-500 font-medium text-sm leading-relaxed border-l-2 border-zinc-200 dark:border-zinc-800 pl-6 italic">Building high-performance organizations through engineering culture.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {pillars.map((pillar, i) => (
              <div key={i} className="group p-8 rounded-[2rem] bg-white dark:bg-[#121212] border border-zinc-200/50 dark:border-zinc-800 hover:border-[#a82a2a]/30 transition-all">
              ...
              {experience.map((job, i) => (
                <div key={i} className="group grid grid-cols-[auto_1fr] gap-8 relative">
                  <div className="flex flex-col items-center pt-1.5">
                    <div className="w-10 h-10 bg-white dark:bg-[#121212] border border-zinc-200 dark:border-zinc-800 rounded-xl flex items-center justify-center z-10 shadow-sm overflow-hidden p-2 group-hover:border-[#a82a2a]/30 transition-all">
                  ...
                  <div className="pb-8">
                    ...
                    <h3 className="text-2xl font-extrabold tracking-tight group-hover:text-[#a82a2a] transition-colors">{job.company}</h3>
                    <p className="text-zinc-500 font-semibold text-sm mb-4 leading-relaxed">{job.role}</p>
                    <div className="grid gap-3">
                      {job.highlights.slice(0, 3).map((item, j) => (
                        <div key={j} className="flex gap-3 text-[15px] text-zinc-600 dark:text-zinc-400 leading-relaxed font-medium">
          ...
          <div className="space-y-12">
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-extrabold tracking-tight">Recent Writing</h2>
                <Link href="/articles" className="text-xs font-bold text-[#a82a2a] hover:underline underline-offset-4">View All</Link>
              </div>
              <div className="space-y-4">
                {articles.slice(0, 3).map((article) => (
                  <Link key={article.slug} href={`/articles/${article.slug}`} className="group block bg-white dark:bg-[#121212] border border-zinc-200/50 dark:border-zinc-800 p-6 rounded-[1.5rem] hover:border-[#a82a2a]/30 transition-all hover:shadow-sm">
                    <time className="text-[10px] font-bold text-[#a82a2a]/60 uppercase tracking-widest mb-2 block">{article.date}</time>
                    <h3 className="text-base font-bold group-hover:text-[#a82a2a] transition-colors leading-snug">{article.title}</h3>
                  </Link>
                ))}
              </div>
            </section>
            ...
            <section className="space-y-6 pt-10 border-t border-zinc-100 dark:border-zinc-800">
              <div className="grid gap-4">
                ...
                <a href={`mailto:${email}`} className="flex items-center justify-center gap-3 w-full py-4 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-2xl font-bold hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all">
